...
class AffordanceFirstPointNet2SSG(nn.Module):
    """
    PointNet++ SSG-based network for per-point affordance prediction of the first hand.

    Input:
      - points_xyz: (B, N, 3) only xyz coordinates.

    Output:
      - logits: (B, 1, N) per-point affordance scores (unnormalized).
    """
    ...
    def __init__(self, use_xyz: bool = True) -> None:
        ...
        self.head = nn.Sequential(
            nn.Conv1d(self.point_feature_channels, 128, kernel_size=1, bias=False),
            nn.BatchNorm1d(128),
...
__all__ = ["AffordanceFirstPointNet2SSG"]
