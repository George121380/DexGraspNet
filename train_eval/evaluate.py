...
def compute_metrics(preds: np.ndarray, targs: np.ndarray) -> dict:
    ...
    # Pearson correlation across all points from all samples
    p = preds.reshape(-1)
    t = targs.reshape(-1)
...
def _make_sample_figure(points_np: np.ndarray, pred_np: np.ndarray, targ_np: np.ndarray, center_np: np.ndarray):
    ...
    for c in [1, 2]:
        fig.update_scenes(
            xaxis_visible=False, yaxis_visible=False, zaxis_visible=False, aspectmode='data', row=1, col=c
        )
    fig.update_layout(title='Affordance Visualization (Prediction vs Target)', title_x=0.5)
    return fig
...
def _save_visualizations(model: torch.nn.Module, loader: DataLoader, device: torch.device,
    ...
    index_lines = ["<html><head><meta charset='utf-8'><title>Affordance Visualizations</title></head><body>",
                   "<h2>Affordance Visualizations</h2>", "<ul>"]
    saved = 0
    model.eval()
    with torch.no_grad():
        for batch_idx, (pts, centers, target) in enumerate(loader):
            pts = pts.to(device)
            centers = centers.to(device)
            target = target.to(device)
...
def _save_overview(model: torch.nn.Module, loader: DataLoader, device: torch.device,
                   out_dir: str, max_vis: int = 36) -> None:
    os.makedirs(out_dir, exist_ok=True)
    html_parts = [
    ...
    with torch.no_grad():
        for batch_idx, (pts, centers, target) in enumerate(loader):
            ...
            for i in range(pts.shape[0]):
                ...
                html_parts.append("<div class='row'><div class='card'><div class='title'>sample %04d</div>%s</div></div>" % (saved, snippet))
                saved += 1
                if saved >= max_vis:
                    break
            if saved >= max_vis:
    ...
    with open(os.path.join(out_dir, 'overview.html'), 'w') as f:
        f.write("\n".join(html_parts))
...
def main():
    ...
    # Build dataset (use full set for evaluation)
    dset = AffordancePairsDataset(
